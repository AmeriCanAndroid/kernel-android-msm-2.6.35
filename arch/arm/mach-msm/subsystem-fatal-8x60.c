...
static void modem_unlock_timeout(struct work_struct *work)
{
	...
	printk(KERN_CRIT "Modem Panic - subsys-restart: Timeout waiting for modem to unwedge.\n");
	arm_pm_restart(RESTART_MODE_MODEM_UNWEDGE_TIMEOUT, "modem-unwedge-timeout");
}
...
static void modem_fatal_fn(struct work_struct *work)
{
	...
	if (modem_state == 0 || modem_state & panic_smsm_states) {
		...
		printk(KERN_CRIT "Modem Panic - subsys-restart: Modem SMSM state = 0x%x!\n", modem_state);
		arm_pm_restart(RESTART_MODE_MODEM_WATCHDOG_BITE, "modem-watchdog-bite");
				...
				void *_cmd)
{
	if (code == MODEM_NOTIFIER_START_RESET) {
		...
		printk(KERN_CRIT "Modem Panic - subsys-restart: Modem error fatal'ed.");
		arm_pm_restart(RESTART_MODE_MODEM_ERROR_FATAL, "modem-error-fatal");
	}
	return NOTIFY_DONE;
}

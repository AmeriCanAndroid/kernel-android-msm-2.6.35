...
/*
TODO:
1. move regulator initialization to shooteru_panel_init()
*/
static struct regulator *l12_3v;
static struct regulator *lvs1_1v8;

static void shooteru_panel_power(int onoff)
{
	static int init;
	int ret;
	int rc;
...
struct mdp_reg shooteru_color_v11[] = {
	{0x93400, 0x0222, 0x0},
	{0x93404, 0xFFE4, 0x0},
	{0x93408, 0xFFFD, 0x0},
	{0x9340C, 0xFFF1, 0x0},
...
int shooteru_mdp_color_enhance(void)
{
	PR_DISP_INFO("%s\n", __func__);
	//mdp_color_enhancement(shooteru_color_v11, ARRAY_SIZE(shooteru_color_v11));

	return 0;
}

int shooteru_mdp_gamma(void)
{
	PR_DISP_INFO("%s\n", __func__);
...
#endif
	.mdp_color_enhance = shooteru_mdp_color_enhance,
	.mdp_gamma = shooteru_mdp_gamma,
};
...
int __init shooteru_init_panel(struct resource *res, size_t size)
{
	...
	mipi_novatek_panel_data.shrink_pwm = shooteru_shrink_pwm;
...
static void shooteru_3Dpanel_on(bool bLandscape)
{
	...
	rc = pm8058_gpio_config(SHOOTERU_3DLCM_PD, &pwm_gpio_config);
	if (rc < 0)
		pr_err("%s pmic gpio config gpio %d failed\n", __func__, SHOOTERU_3DLCM_PD);

	rc = pm8058_gpio_config(SHOOTERU_3DCLK, &clk_gpio_config_on);
	if (rc < 0)
		pr_err("%s pmic gpio config gpio %d failed\n", __func__, SHOOTERU_3DCLK);
	...
	if(bLandscape) {
		...
		gpio_set_value(SHOOTERU_CTL_3D_1, 1);
		gpio_set_value(SHOOTERU_CTL_3D_2, 1);
		gpio_set_value(SHOOTERU_CTL_3D_3, 1);
		gpio_set_value(SHOOTERU_CTL_3D_4, 0);
	} else {
		mdp_color_enhancement(mdp_sharp_barrier_on, ARRAY_SIZE(mdp_sharp_barrier_on));
		gpio_set_value(SHOOTERU_CTL_3D_1, 1);
		gpio_set_value(SHOOTERU_CTL_3D_2, 1);
		gpio_set_value(SHOOTERU_CTL_3D_3, 0);
		gpio_set_value(SHOOTERU_CTL_3D_4, 1);
	}
}

static void shooteru_3Dpanel_off(void)
{
	int rc;
	...
	rc = pm8058_gpio_config(SHOOTERU_3DCLK, &clk_gpio_config_off);
	if (rc < 0)
		pr_err("%s pmic gpio config gpio %d failed\n", __func__, SHOOTERU_3DCLK);
	gpio_set_value(SHOOTERU_CTL_3D_1, 0);
	gpio_set_value(SHOOTERU_CTL_3D_2, 0);
	gpio_set_value(SHOOTERU_CTL_3D_3, 0);
	gpio_set_value(SHOOTERU_CTL_3D_4, 0);
		...
		struct device_attribute *attr, const char *buf, size_t count)
{
	...
	if ((buf[0] == '0' || buf[0] == '1' || buf[0] == '2')
		...
		switch (val) {
		case LANDSCAPE_3D:
			shooteru_3Dpanel_on(true);
			break;
		case PORTRAIT_3D:
			shooteru_3Dpanel_on(false);
			break;
		case OFF_3D:
			shooteru_3Dpanel_off();
			break;
		default:
			break;
		}
...
static int shooteru_3Dpanel_probe(struct platform_device * pdev)
{
	int err = 0;
...
static int shooteru_3Dpanel_remove(struct platform_device * pdev)
{
	printk(KERN_INFO "%s\n", __func__);
	if(pwm_3d) {
		pwm_free(pwm_3d);
...
struct platform_driver shooteru_3Dpanel_driver = {
	.probe	= shooteru_3Dpanel_probe,
	.remove	= shooteru_3Dpanel_remove,
	.driver	= {
		.name = "panel_3d",
	},
};

static int __init shooteru_3Dpanel_init(void)
{
	pr_info("%s(%d)\n", __func__, __LINE__);
	return platform_driver_register(&shooteru_3Dpanel_driver);
}

static void __exit shooteru_3Dpanel_exit(void)
{
	platform_driver_unregister(&shooteru_3Dpanel_driver);
}

module_init(shooteru_3Dpanel_init);
module_exit(shooteru_3Dpanel_exit);

/* linux/arch/arm/mach-msm/board-shooteru-wifi.c
*/
...
int shooteru_wifi_power(int on);
int shooteru_wifi_reset(int on);
int shooteru_wifi_set_carddetect(int on);
int shooteru_wifi_get_mac_addr(unsigned char *buf);
...
static void *shooteru_wifi_mem_prealloc(int section, unsigned long size)
{
...
int __init shooteru_init_wifi_mem(void)
{
	int i;
...
static struct resource shooteru_wifi_resources[] = {
	[0] = {
		.name		= "bcm4329_wlan_irq",
		.start		= MSM_GPIO_TO_INT(SHOOTERU_GPIO_WIFI_IRQ),
		.end		= MSM_GPIO_TO_INT(SHOOTERU_GPIO_WIFI_IRQ),
#ifdef CONFIG_BCM4329_PURE_ANDROID
...
static struct wifi_platform_data shooteru_wifi_control = {
	.set_power      = shooteru_wifi_power,
	.set_reset      = shooteru_wifi_reset,
	.set_carddetect = shooteru_wifi_set_carddetect,
	.mem_prealloc   = shooteru_wifi_mem_prealloc,
	.get_mac_addr	= shooteru_wifi_get_mac_addr,
...
static struct platform_device shooteru_wifi_device = {
	.name           = "bcm4329_wlan",
	.id             = 1,
	.num_resources  = ARRAY_SIZE(shooteru_wifi_resources),
	.resource       = shooteru_wifi_resources,
	.dev            = {
		.platform_data = &shooteru_wifi_control,
	},
};

extern unsigned char *get_wifi_nvs_ram(void);

static unsigned shooteru_wifi_update_nvs(char *str)
{
...
int shooteru_wifi_get_mac_addr(unsigned char *buf)
{
	...
	printk("shooteru_wifi_get_mac_addr = %02x %02x %02x %02x %02x %02x \n",
...
#else
	shooteru_wifi_update_nvs("sd_oobonly=1\n");
#endif
	shooteru_wifi_update_nvs("btc_params80=0\n");
	shooteru_wifi_update_nvs("btc_params6=30\n");
	shooteru_init_wifi_mem();
	ret = platform_device_register(&shooteru_wifi_device);
	return ret;
}

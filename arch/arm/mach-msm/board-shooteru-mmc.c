/* linux/arch/arm/mach-msm/board-shooteru-mmc.c
 *
...
static uint32_t wifi_on_gpio_table[] = {
	...
	GPIO_CFG(SHOOTERU_GPIO_WIFI_IRQ, 0, GPIO_CFG_INPUT, GPIO_CFG_NO_PULL, GPIO_CFG_4MA), /* WLAN IRQ */
};
...
static uint32_t wifi_off_gpio_table[] = {
	...
	GPIO_CFG(SHOOTERU_GPIO_WIFI_IRQ, 0, GPIO_CFG_INPUT, GPIO_CFG_PULL_DOWN, GPIO_CFG_4MA), /* WLAN IRQ */
};
 ...
 */
static struct embedded_sdio_data shooteru_wifi_emb_data = {
	.cccr	= {
		.sdio_vsn	= 2,
		.multi_block	= 1,
		.low_speed	= 0,
...
static int
shooteru_wifi_status_register(void (*callback)(int card_present, void *dev_id),
				void *dev_id)
{
	if (wifi_status_cb)
		return -EAGAIN;
...
static int shooteru_wifi_cd;	/* WiFi virtual 'card detect' status */

static unsigned int shooteru_wifi_status(struct device *dev)
{
	return shooteru_wifi_cd;
}

static struct mmc_platform_data shooteru_wifi_data = {
        .ocr_mask               = MMC_VDD_28_29,
        .status                 = shooteru_wifi_status,
        .register_status_notify = shooteru_wifi_status_register,
        .embedded_sdio          = &shooteru_wifi_emb_data,
...
int shooteru_wifi_set_carddetect(int val)
{
	printk(KERN_INFO "%s: %d\n", __func__, val);
	shooteru_wifi_cd = val;
	if (wifi_status_cb)
...
EXPORT_SYMBOL(shooteru_wifi_set_carddetect);
...
int shooteru_wifi_power(int on)
{
	...
	gpio_set_value(SHOOTERU_GPIO_WIFI_SHUTDOWN_N, on); /* WIFI_SHUTDOWN */

	mdelay(120);
	return 0;
}
EXPORT_SYMBOL(shooteru_wifi_power);
...
/* ---- MMC ---- */
int __init shooteru_init_mmc()
{
	uint32_t id;
	wifi_status_cb = NULL;

	printk(KERN_INFO "shooteru: %s\n", __func__);
   ...
   //config_gpio_table(wimax_uart_off_gpio_table,
	...
	id = GPIO_CFG(SHOOTERU_GPIO_WIFI_SHUTDOWN_N, 0, GPIO_CFG_OUTPUT,
		GPIO_CFG_NO_PULL, GPIO_CFG_2MA);
	gpio_tlmm_config(id, 0);
	gpio_set_value(SHOOTERU_GPIO_WIFI_SHUTDOWN_N, 0);

	msm_add_sdcc(4, &shooteru_wifi_data);

...
#ifdef CONFIG_PERFLOCK
static unsigned shooteru_perf_acpu_table[] = {
	384000000,
	756000000,
	1188000000,
};

static struct perflock_platform_data shooteru_perflock_data = {
	.perf_acpu_table = shooteru_perf_acpu_table,
	.table_size = ARRAY_SIZE(shooteru_perf_acpu_table),
};
#endif
...
static int shooteru_phy_init_seq[] = { 0x06, 0x36, 0x0C, 0x31, 0x31, 0x32, 0x1, 0x0E, 0x1, 0x11, -1 };
...
static uint32_t usb_uart_input_pulldown_switch_table[] = {
	GPIO_CFG(SHOOTERU_GPIO_MHL_USB_EN, 0, GPIO_CFG_INPUT,
		GPIO_CFG_PULL_DOWN, GPIO_CFG_2MA),
};
/*
static uint32_t usb_uart_switch_table[] = {
	GPIO_CFG(SHOOTERU_GPIO_MHL_USB_EN, 0, GPIO_CFG_OUTPUT,
		GPIO_CFG_NO_PULL, GPIO_CFG_2MA),
};
*/
static uint32_t mhl_usb_switch_table[] = {
	GPIO_CFG(SHOOTERU_GPIO_MHL_USB_SW, 0, GPIO_CFG_OUTPUT,
		GPIO_CFG_NO_PULL, GPIO_CFG_2MA),
};


static void config_shooteru_usb_uart_gpios(int uart)
{
	/*HW has auto switch of usb/uart*/
}

static struct msm_hsusb_platform_data msm_hsusb_pdata = {
	.phy_init_seq		= shooteru_phy_init_seq,
	.ldo_init		 = msm_hsusb_ldo_init,
	.ldo_enable		 = msm_hsusb_ldo_enable,
	.usb_uart_switch	= config_shooteru_usb_uart_gpios,
...
static uint32_t usb_ID_PIN_input_table[] = {
	GPIO_CFG(SHOOTERU_GPIO_USB_ID, 0, GPIO_CFG_INPUT, GPIO_CFG_NO_PULL, GPIO_CFG_2MA),
};

static uint32_t usb_ID_PIN_ouput_table[] = {
	GPIO_CFG(SHOOTERU_GPIO_USB_ID, 0, GPIO_CFG_OUTPUT, GPIO_CFG_NO_PULL, GPIO_CFG_2MA),
};

void config_shooteru_usb_id_gpios(bool output)
{
	if (output) {
		config_gpio_table(usb_ID_PIN_ouput_table, ARRAY_SIZE(usb_ID_PIN_ouput_table));
		gpio_set_value(SHOOTERU_GPIO_USB_ID, 1);
		printk(KERN_INFO "%s %d output high\n",  __func__, SHOOTERU_GPIO_USB_ID);
	} else {
...
static uint32_t mhl_reset_pin_ouput_table[] = {
	GPIO_CFG(SHOOTERU_GPIO_MHL_RESET, 0, GPIO_CFG_OUTPUT, GPIO_CFG_NO_PULL, GPIO_CFG_4MA),
};

static uint32_t mhl_usb_switch_ouput_table[] = {
	GPIO_CFG(SHOOTERU_GPIO_MHL_USB_SW, 0, GPIO_CFG_OUTPUT, GPIO_CFG_NO_PULL, GPIO_CFG_4MA),
};

void config_shooteru_mhl_gpios(void)
{
...
static void shooteru_usb_dpdn_switch(int path)
{
	switch (path) {
	case PATH_USB:
	case PATH_MHL:
	{
		...
		gpio_set_value(SHOOTERU_GPIO_MHL_USB_SW, (mhl ^ !polarity) ? 1 : 0);
		break;
	}
	}
...
static struct cable_detect_platform_data cable_detect_pdata = {
	...
	.usb_id_pin_gpio = SHOOTERU_GPIO_USB_ID,
	.usb_dpdn_switch = shooteru_usb_dpdn_switch,
	.mhl_reset_gpio = SHOOTERU_GPIO_MHL_RESET,
	.mpp_data = {
		.usbid_mpp	=  XOADC_MPP_4,
		.usbid_amux	= PM_MPP_AIN_AMUX_CH5,
	},
	.config_usb_id_gpios = config_shooteru_usb_id_gpios,
#ifdef CONFIG_FB_MSM_HDMI_MHL
...
static void config_flashlight_gpios(void)
{
	static uint32_t flashlight_gpio_table[] = {
		GPIO_CFG(SHOOTERU_TORCH_EN, 0, GPIO_CFG_OUTPUT,
			GPIO_CFG_NO_PULL, GPIO_CFG_2MA),
		GPIO_CFG(SHOOTERU_FLASH_EN, 0, GPIO_CFG_OUTPUT,
			GPIO_CFG_NO_PULL, GPIO_CFG_2MA),
	};
...
static struct flashlight_platform_data flashlight_data = {
	...
	.torch = SHOOTERU_TORCH_EN,
	.flash = SHOOTERU_FLASH_EN,
	.torch_set1 = PM8058_GPIO_PM_TO_SYS(SHOOTERU_TORCH_SET1),
	.torch_set2 = PM8058_GPIO_PM_TO_SYS(SHOOTERU_TORCH_SET2),
...
static uint32_t camera_off_gpio_table_sp3d[] = {
	GPIO_CFG(SHOOTERU_CAM_I2C_SDA	, 1, GPIO_CFG_OUTPUT, GPIO_CFG_NO_PULL, GPIO_CFG_8MA),/*i2c*/
	GPIO_CFG(SHOOTERU_CAM_I2C_SCL	, 1, GPIO_CFG_OUTPUT, GPIO_CFG_NO_PULL, GPIO_CFG_8MA),/*i2c*/
	GPIO_CFG(SHOOTERU_SP3D_MCLK		, 0, GPIO_CFG_OUTPUT, GPIO_CFG_NO_PULL, GPIO_CFG_2MA),/*MCLK*/
	GPIO_CFG(SHOOTERU_SP3D_INT		, 0, GPIO_CFG_INPUT, GPIO_CFG_PULL_DOWN, GPIO_CFG_2MA),/*sharp INT*/
	GPIO_CFG(SHOOTERU_SP3D_SPI_DO	, 0, GPIO_CFG_OUTPUT, GPIO_CFG_NO_PULL, GPIO_CFG_2MA),
	GPIO_CFG(SHOOTERU_SP3D_SPI_DI	, 0, GPIO_CFG_INPUT, GPIO_CFG_PULL_DOWN, GPIO_CFG_2MA),
	GPIO_CFG(SHOOTERU_SP3D_SPI_CS	, 0, GPIO_CFG_OUTPUT, GPIO_CFG_NO_PULL, GPIO_CFG_2MA),
	GPIO_CFG(SHOOTERU_SP3D_SPI_CLK	, 0, GPIO_CFG_OUTPUT, GPIO_CFG_NO_PULL, GPIO_CFG_2MA),
	GPIO_CFG(SHOOTERU_SP3D_GATE		, 0, GPIO_CFG_OUTPUT, GPIO_CFG_NO_PULL, GPIO_CFG_2MA),
	GPIO_CFG(SHOOTERU_SP3D_CORE_GATE	, 0, GPIO_CFG_OUTPUT, GPIO_CFG_NO_PULL, GPIO_CFG_2MA),
	GPIO_CFG(SHOOTERU_SP3D_SYS_RST	, 0, GPIO_CFG_OUTPUT, GPIO_CFG_NO_PULL, GPIO_CFG_2MA),
	GPIO_CFG(SHOOTERU_SP3D_PDX		, 0, GPIO_CFG_OUTPUT, GPIO_CFG_NO_PULL, GPIO_CFG_2MA),
	GPIO_CFG(SHOOTERU_WEBCAM_RST	, 0, GPIO_CFG_OUTPUT, GPIO_CFG_NO_PULL, GPIO_CFG_2MA),/*camera reset*/
	GPIO_CFG(SHOOTERU_WEBCAM_STB	, 0, GPIO_CFG_OUTPUT, GPIO_CFG_NO_PULL, GPIO_CFG_2MA),/*camera standby*/
	GPIO_CFG(SHOOTERU_CAM_SEL		, 0, GPIO_CFG_OUTPUT, GPIO_CFG_NO_PULL, GPIO_CFG_2MA),/*camera switch*/
};
...
static uint32_t camera_off_gpio_table_liteon[] = {
	...
	GPIO_CFG(SHOOTERU_SP3D_MCLK		, 0, GPIO_CFG_OUTPUT, GPIO_CFG_NO_PULL, GPIO_CFG_2MA),/*MCLK*/
	GPIO_CFG(SHOOTERU_SP3D_INT		, 0, GPIO_CFG_INPUT, GPIO_CFG_PULL_DOWN, GPIO_CFG_2MA),/*sharp INT*/
	GPIO_CFG(SHOOTERU_SP3D_SPI_DO	, 0, GPIO_CFG_INPUT, GPIO_CFG_NO_PULL, GPIO_CFG_2MA),
	GPIO_CFG(SHOOTERU_SP3D_SPI_DI	, 0, GPIO_CFG_INPUT, GPIO_CFG_PULL_DOWN, GPIO_CFG_2MA),
	GPIO_CFG(SHOOTERU_SP3D_SPI_CS	, 0, GPIO_CFG_INPUT, GPIO_CFG_NO_PULL, GPIO_CFG_2MA),
	GPIO_CFG(SHOOTERU_SP3D_SPI_CLK	, 0, GPIO_CFG_INPUT, GPIO_CFG_NO_PULL, GPIO_CFG_2MA),
	GPIO_CFG(SHOOTERU_S5K4E1_VCM_PD, 0, GPIO_CFG_OUTPUT, GPIO_CFG_NO_PULL, GPIO_CFG_2MA),
	GPIO_CFG(SHOOTERU_S5K4E1_INTB, 0, GPIO_CFG_INPUT, GPIO_CFG_NO_PULL, GPIO_CFG_2MA),
	GPIO_CFG(SHOOTERU_S5K4E1_PD, 0, GPIO_CFG_OUTPUT, GPIO_CFG_NO_PULL, GPIO_CFG_2MA),
	GPIO_CFG(SHOOTERU_WEBCAM_RST	, 0, GPIO_CFG_OUTPUT, GPIO_CFG_NO_PULL, GPIO_CFG_2MA),/*camera reset*/
	GPIO_CFG(SHOOTERU_WEBCAM_STB	, 0, GPIO_CFG_OUTPUT, GPIO_CFG_NO_PULL, GPIO_CFG_2MA),/*camera standby*/
	GPIO_CFG(SHOOTERU_CAM_SEL		, 0, GPIO_CFG_OUTPUT, GPIO_CFG_NO_PULL, GPIO_CFG_2MA),/*camera switch*/
};
...
static uint32_t camera_on_gpio_table_sp3d[] = {
	GPIO_CFG(SHOOTERU_CAM_I2C_SDA	, 1, GPIO_CFG_OUTPUT, GPIO_CFG_NO_PULL, GPIO_CFG_8MA),
	GPIO_CFG(SHOOTERU_CAM_I2C_SCL	, 1, GPIO_CFG_OUTPUT, GPIO_CFG_NO_PULL, GPIO_CFG_8MA),
	GPIO_CFG(SHOOTERU_SP3D_MCLK		, 1, GPIO_CFG_OUTPUT, GPIO_CFG_NO_PULL, GPIO_CFG_2MA),/*MCLK*/
	GPIO_CFG(SHOOTERU_SP3D_INT		, 0, GPIO_CFG_INPUT, GPIO_CFG_NO_PULL, GPIO_CFG_2MA),
	GPIO_CFG(SHOOTERU_SP3D_SPI_DO	, 1, GPIO_CFG_OUTPUT, GPIO_CFG_NO_PULL, GPIO_CFG_2MA),
	GPIO_CFG(SHOOTERU_SP3D_SPI_DI	, 1, GPIO_CFG_INPUT, GPIO_CFG_NO_PULL, GPIO_CFG_2MA),
	GPIO_CFG(SHOOTERU_SP3D_SPI_CS	, 1, GPIO_CFG_OUTPUT, GPIO_CFG_NO_PULL, GPIO_CFG_2MA),
	GPIO_CFG(SHOOTERU_SP3D_SPI_CLK	, 1, GPIO_CFG_OUTPUT, GPIO_CFG_NO_PULL, GPIO_CFG_2MA),
	GPIO_CFG(SHOOTERU_SP3D_GATE		, 0, GPIO_CFG_OUTPUT, GPIO_CFG_NO_PULL, GPIO_CFG_2MA),
	GPIO_CFG(SHOOTERU_SP3D_CORE_GATE, 0, GPIO_CFG_OUTPUT, GPIO_CFG_NO_PULL, GPIO_CFG_2MA),
	GPIO_CFG(SHOOTERU_SP3D_SYS_RST	, 0, GPIO_CFG_OUTPUT, GPIO_CFG_NO_PULL, GPIO_CFG_2MA),
	GPIO_CFG(SHOOTERU_SP3D_PDX		, 0, GPIO_CFG_OUTPUT, GPIO_CFG_NO_PULL, GPIO_CFG_2MA),
	GPIO_CFG(SHOOTERU_WEBCAM_RST	, 0, GPIO_CFG_OUTPUT, GPIO_CFG_NO_PULL, GPIO_CFG_2MA),
	GPIO_CFG(SHOOTERU_WEBCAM_STB	, 0, GPIO_CFG_OUTPUT, GPIO_CFG_NO_PULL, GPIO_CFG_2MA),
	GPIO_CFG(SHOOTERU_CAM_SEL		, 0, GPIO_CFG_OUTPUT, GPIO_CFG_NO_PULL, GPIO_CFG_2MA),
};
...
static uint32_t camera_on_gpio_table_liteon[] = {
	...
	GPIO_CFG(SHOOTERU_SP3D_INT		, 0, GPIO_CFG_INPUT, GPIO_CFG_NO_PULL, GPIO_CFG_2MA),
	GPIO_CFG(SHOOTERU_SP3D_SPI_DO	, 0, GPIO_CFG_INPUT, GPIO_CFG_NO_PULL, GPIO_CFG_2MA),
	GPIO_CFG(SHOOTERU_SP3D_SPI_DI	, 0, GPIO_CFG_INPUT, GPIO_CFG_NO_PULL, GPIO_CFG_2MA),
	GPIO_CFG(SHOOTERU_SP3D_SPI_CS	, 0, GPIO_CFG_INPUT, GPIO_CFG_NO_PULL, GPIO_CFG_2MA),
	GPIO_CFG(SHOOTERU_SP3D_SPI_CLK	, 0, GPIO_CFG_INPUT, GPIO_CFG_NO_PULL, GPIO_CFG_2MA),
	GPIO_CFG(SHOOTERU_S5K4E1_VCM_PD, 0, GPIO_CFG_OUTPUT, GPIO_CFG_PULL_DOWN, GPIO_CFG_8MA),
	GPIO_CFG(SHOOTERU_S5K4E1_INTB, 0, GPIO_CFG_INPUT, GPIO_CFG_PULL_DOWN, GPIO_CFG_8MA),
	GPIO_CFG(SHOOTERU_S5K4E1_PD, 0, GPIO_CFG_OUTPUT, GPIO_CFG_PULL_DOWN, GPIO_CFG_8MA),
	GPIO_CFG(SHOOTERU_WEBCAM_RST	, 0, GPIO_CFG_OUTPUT, GPIO_CFG_NO_PULL, GPIO_CFG_2MA),
	GPIO_CFG(SHOOTERU_WEBCAM_STB	, 0, GPIO_CFG_OUTPUT, GPIO_CFG_NO_PULL, GPIO_CFG_2MA),
	GPIO_CFG(SHOOTERU_CAM_SEL		, 0, GPIO_CFG_OUTPUT, GPIO_CFG_NO_PULL, GPIO_CFG_2MA),
};
...
static uint32_t sp3d_spi_gpio[] = {
	...
	GPIO_CFG(SHOOTERU_SP3D_SPI_DO,  1, GPIO_CFG_OUTPUT, GPIO_CFG_PULL_DOWN, GPIO_CFG_8MA),
	GPIO_CFG(SHOOTERU_SP3D_SPI_DI,  1, GPIO_CFG_INPUT, GPIO_CFG_PULL_DOWN, GPIO_CFG_8MA),
	GPIO_CFG(SHOOTERU_SP3D_SPI_CS,  1, GPIO_CFG_OUTPUT, GPIO_CFG_PULL_UP, GPIO_CFG_8MA),
	GPIO_CFG(SHOOTERU_SP3D_SPI_CLK, 1, GPIO_CFG_OUTPUT, GPIO_CFG_PULL_DOWN, GPIO_CFG_8MA),
};
#endif
...
static void shooteru_set_gpio(int gpio, int state){
	gpio_set_value(gpio, state);
}

#ifdef CONFIG_SP3D
static int Shooteru_sp3d_vreg_on(void)
{
	int rc;
	pr_info("[Camera]%s %d\n", __func__, system_rev);
	/* VDDIO*/
	...
	shooteru_set_gpio(SHOOTERU_SP3D_CORE_GATE, 1); // CORE GATE
	shooteru_set_gpio(SHOOTERU_SP3D_SYS_RST, 1); // RST
	shooteru_set_gpio(SHOOTERU_SP3D_PDX, 1); // PDX
	shooteru_set_gpio(SHOOTERU_SP3D_GATE, 1); // GATE
	/* main camera AVDD */
...
static int Shooteru_sp3d_vreg_off(void)
{
	int rc;
	pr_info("[Camera]%s\n", __func__);
	shooteru_set_gpio(SHOOTERU_SP3D_PDX, 0); // PDX
	/* main camera MVDD */
	...
	shooteru_set_gpio(SHOOTERU_SP3D_SYS_RST, 0); // RST
	shooteru_set_gpio(SHOOTERU_SP3D_CORE_GATE, 0); // CORE GATE
	shooteru_set_gpio(SHOOTERU_SP3D_GATE, 0); // GATE
...
static int Shooteru_qs_s5k4e1_vreg_on(void)
{
	int rc;
	pr_info("[Camera]%s %x\n", __func__, system_rev);
	mdelay(50);
	...
	shooteru_set_gpio(SHOOTERU_S5K4E1_INTB, 1);
	shooteru_set_gpio(SHOOTERU_S5K4E1_PD, 1);
	shooteru_set_gpio(SHOOTERU_S5K4E1_VCM_PD, 1);

	return rc;
}

static int Shooteru_qs_s5k4e1_vreg_off(void)
{
	int rc;
	pr_info("[Camera]%s\n", __func__);
	shooteru_set_gpio(SHOOTERU_S5K4E1_INTB, 0); // interrupt
	shooteru_set_gpio(SHOOTERU_S5K4E1_VCM_PD, 0); // PDX
	shooteru_set_gpio(SHOOTERU_S5K4E1_PD, 0); // RST
...
static int Shooteru_s5k6aafx_vreg_on(void)
{
	int rc;
	pr_info("[Camera]%s\n", __func__);
...
static int Shooteru_s5k6aafx_vreg_off(void)
{
	int rc;
	pr_info("[Camera]%s\n", __func__);
...
static void Shooteru_maincam_clk_switch(void)
{
	pr_info("[Camera]Doing clk switch (Main Cam)\n");
	gpio_set_value(SHOOTERU_CAM_SEL, 0);
}

static void Shooteru_seccam_clk_switch(void)
{
	pr_info("Doing clk switch (2nd Cam)\n");
	gpio_set_value(SHOOTERU_CAM_SEL, 1);
}
#ifdef CONFIG_HTC_BATT8x60
static struct htc_battery_platform_data htc_battery_pdev_data = {
	.guage_driver = GUAGE_NONE,
	.gpio_mbat_in = MSM_GPIO_TO_INT(SHOOTERU_GPIO_MBAT_IN),
...
static void shooteru_config_camera_on_gpios(void)
{
	if (system_rev == 0x80 && engineerid == 0x1) {
...
static void shooteru_config_camera_off_gpios(void)
{
	if (system_rev == 0x80 && engineerid == 0x1) {
	...
	shooteru_set_gpio(SHOOTERU_SP3D_SPI_DO, 0);
	shooteru_set_gpio(SHOOTERU_SP3D_SPI_CS, 0);
	shooteru_set_gpio(SHOOTERU_SP3D_SPI_CLK, 0);
	shooteru_set_gpio(SHOOTERU_SP3D_MCLK, 0);
	shooteru_set_gpio(SHOOTERU_CAM_SEL, 0);
}

#define GPIO_CAM_EN_WEB_CAM (GPIO_EXPANDER_GPIO_BASE + (16 * 3) + 8 + 4)
static struct msm_camera_device_platform_data msm_camera_device_data = {
	.camera_gpio_on  = shooteru_config_camera_on_gpios,
	.camera_gpio_off = shooteru_config_camera_off_gpios,
...
static struct msm_camera_sensor_info msm_camera_sensor_sp3d_data = {
	.sensor_name	= "sp3d",
	.vcm_enable		= 0,
	.camera_power_on = Shooteru_sp3d_vreg_on,
	.camera_power_off = Shooteru_sp3d_vreg_off,
	.camera_clk_switch = Shooteru_maincam_clk_switch,
...
static struct msm_camera_sensor_info msm_camera_sensor_qs_s5k4e1_data = {
	...
	.sensor_reset	= SHOOTERU_S5K4E1_PD,
	.vcm_enable		= 0,
	.camera_power_on = Shooteru_qs_s5k4e1_vreg_on,
	.camera_power_off = Shooteru_qs_s5k4e1_vreg_off,
	.camera_clk_switch = Shooteru_maincam_clk_switch,
	.pdata			= &msm_camera_device_data,
	.resource		= msm_camera_resources,
	.num_resources	= ARRAY_SIZE(msm_camera_resources),
...
static struct msm_camera_sensor_info msm_camera_sensor_s5k6aafx_data = {
	...
	.sensor_reset	= SHOOTERU_WEBCAM_RST,/*2nd Cam RST*/
	.sensor_pwd		= SHOOTERU_WEBCAM_STB,/*2nd Cam PWD*/
	.vcm_enable		= 0,
	.camera_power_on = Shooteru_s5k6aafx_vreg_on,
	.camera_power_off = Shooteru_s5k6aafx_vreg_off,
	.camera_clk_switch = Shooteru_seccam_clk_switch,
...
static T_MHL_PLATFORM_DATA mhl_sii9234_device_data = {
	...
	#ifdef CONFIG_FB_MSM_HDMI_MHL
	.mhl_usb_switch		= shooteru_usb_dpdn_switch,
	.mhl_1v2_power = mhl_sii9234_1v2_power,
	#endif
};
...
static struct tps65200_platform_data tps65200_data = {
	.gpio_chg_stat = PM8058_GPIO_IRQ(PM8058_IRQ_BASE, SHOOTERU_CHG_STAT),
	.gpio_chg_int  = MSM_GPIO_TO_INT(SHOOTERU_GPIO_CHG_INT),
};
...
static uint32_t gsbi5_gpio_table[] = {
	GPIO_CFG(SHOOTERU_TP_I2C_SDA, 1, GPIO_CFG_INPUT, GPIO_CFG_NO_PULL, GPIO_CFG_8MA),
	GPIO_CFG(SHOOTERU_TP_I2C_SCL, 1, GPIO_CFG_INPUT, GPIO_CFG_NO_PULL, GPIO_CFG_8MA),
};

static uint32_t gsbi7_gpio_table[] = {
	GPIO_CFG(SHOOTERU_GENERAL_I2C_SDA, 1, GPIO_CFG_INPUT, GPIO_CFG_NO_PULL, GPIO_CFG_8MA),
	GPIO_CFG(SHOOTERU_GENERAL_I2C_SCL, 1, GPIO_CFG_INPUT, GPIO_CFG_NO_PULL, GPIO_CFG_8MA),
};

static uint32_t gsbi10_gpio_table[] = {
	GPIO_CFG(SHOOTERU_SENSOR_I2C_SDA, 1, GPIO_CFG_INPUT, GPIO_CFG_NO_PULL, GPIOMUX_DRV_8MA),
	GPIO_CFG(SHOOTERU_SENSOR_I2C_SCL, 1, GPIO_CFG_INPUT, GPIO_CFG_NO_PULL, GPIOMUX_DRV_8MA),
};
...
static struct platform_device shooteru_3Dpanel_device = {
	.name = "panel_3d",
	.id = -1,
};

#ifdef CONFIG_BT
static struct platform_device shooteru_rfkill = {
	.name = "shooteru_rfkill",
	.id = -1,
};

static struct htc_sleep_clk_platform_data htc_slp_clk_data = {
	.sleep_clk_pin = SHOOTERU_WIFI_BT_SLEEP_CLK,
...
static struct msm_serial_hs_platform_data msm_uart_dm1_pdata = {
	.rx_wakeup_irq = MSM_GPIO_TO_INT(SHOOTERU_GPIO_BT_HOST_WAKE),
	.inject_rx_on_wakeup = 0,
	.cpu_lock_supported = 0,

	/* for bcm */
	.bt_wakeup_pin_supported = 1,
	.bt_wakeup_pin = SHOOTERU_GPIO_BT_CHIP_WAKE,
	.host_wakeup_pin = SHOOTERU_GPIO_BT_HOST_WAKE,
};
#endif
...
static struct htc_headset_gpio_platform_data htc_headset_gpio_data = {
	.hpin_gpio		= SHOOTERU_GPIO_AUD_HP_DET,
	.key_enable_gpio	= 0,
	.mic_select_gpio	= 0,
};
...
static struct tpa2051d3_platform_data tpa2051d3_pdata = {
	.gpio_tpa2051_spk_en = SHOOTERU_AUD_SPK_ENO,
...
static int shooteru_ts_atmel_power(int on)
{
	pr_info("%s: power %d\n", __func__, on);

	gpio_set_value(PM8058_GPIO_PM_TO_SYS(SHOOTERU_TP_RST), 0);
	msleep(5);
	gpio_set_value(PM8058_GPIO_PM_TO_SYS(SHOOTERU_TP_RST), 1);
	msleep(40);

	return 0;
}

struct atmel_i2c_platform_data shooteru_ts_atmel_data[] = {
	{
		.version = 0x020,
		...
		.abs_width_min = 0,
		.abs_width_max = 20,
		.gpio_irq = SHOOTERU_TP_ATT_N,
		.power = shooteru_ts_atmel_power,
...
static struct i2c_board_info msm_i2c_gsbi5_info[] = {
	{
		...
		.platform_data = &shooteru_ts_atmel_data,
		.irq = MSM_GPIO_TO_INT(SHOOTERU_TP_ATT_N)
	},
};
...
/*
static int config_shooteru_proximity_gpios(int on)
{
	int ret;
	...
	ret = pm8058_gpio_config(SHOOTERU_PS_VOUT, &proximity_INT_gpio_cfg);
	if (ret)
...
static int __isl29028_power(int on)
{
	...
	if (on) {
		config_shooteru_proximity_gpios(1);
		...
		config_shooteru_proximity_gpios(0);
	}
...
static struct isl29028_platform_data isl29028_pdata = {
	.intr = PM8058_GPIO_PM_TO_SYS(SHOOTERU_PS_VOUT),
...
static struct i2c_board_info i2c_isl29028_devices[] = {
	{
		...
		.irq = PM8058_GPIO_IRQ(PM8058_IRQ_BASE, SHOOTERU_PS_VOUT),
	},
};
...
#endif
#ifdef CONFIG_SERIAL_MSM_HS
	msm_uart_dm1_pdata.rx_wakeup_irq = gpio_to_irq(SHOOTERU_GPIO_BT_HOST_WAKE);
...
static void __init shooteru_map_io(void)
{
...
*/	 
	// Follow up in  shooteru_init_mmc()
...
static uint32_t msm_spi_gpio[] = {
	GPIO_CFG(SHOOTERU_SPI_DO,  0, GPIO_CFG_OUTPUT, GPIO_CFG_NO_PULL, GPIO_CFG_2MA),
	GPIO_CFG(SHOOTERU_SPI_DI,  0, GPIO_CFG_INPUT, GPIO_CFG_PULL_DOWN, GPIO_CFG_2MA),
	GPIO_CFG(SHOOTERU_SPI_CS,  0, GPIO_CFG_OUTPUT, GPIO_CFG_NO_PULL, GPIO_CFG_2MA),
	GPIO_CFG(SHOOTERU_SPI_CLK, 0, GPIO_CFG_OUTPUT, GPIO_CFG_NO_PULL, GPIO_CFG_2MA),
};
...
static ssize_t shooteru_virtual_keys_show(struct kobject *kobj,
...
static struct kobj_attribute shooteru_virtual_keys_attr = {
	.attr = {
		.name = "virtualkeys.atmel-touchscreen",
		.mode = S_IRUGO,
	},
	.show = &shooteru_virtual_keys_show,
};

static struct attribute *shooteru_properties_attrs[] = {
	&shooteru_virtual_keys_attr.attr,
	NULL
};

static struct attribute_group shooteru_properties_attr_group = {
	.attrs = shooteru_properties_attrs,
};
...
#ifdef CONFIG_USB_ANDROID
static void shooteru_add_usb_devices(void)
{
	printk("%s\n", __func__);
	...
	msm_otg_pdata.idgnd_gpio = SHOOTERU_GPIO_USB_ID;
	msm_device_otg.dev.platform_data = &msm_otg_pdata;
	...
	config_shooteru_usb_uart_gpios(0);
	config_shooteru_usb_id_gpios(0);
	config_shooteru_mhl_gpios();

	config_gpio_table(mhl_usb_switch_table, ARRAY_SIZE(mhl_usb_switch_table));

	gpio_set_value(SHOOTERU_GPIO_MHL_USB_SW, 0);
...
static void __init shooteru_init(void)
{
	int rc = 0;
	...
	pr_err("shooteru_init, system_rev = %d\n", system_rev);
...
#ifdef CONFIG_PERFLOCK
	perflock_init(&shooteru_perflock_data);
#endif
	...
	if (system_rev >= 1) {
		htc_headset_pmic_data.key_gpio =
			PM8058_GPIO_PM_TO_SYS(SHOOTERU_AUD_REMO_PRES);
		htc_headset_pmic_data.key_enable_gpio =
			PM8058_GPIO_PM_TO_SYS(SHOOTERU_AUD_REMO_EN);
	...
	shooteru_init_panel(msm_fb_resources, ARRAY_SIZE(msm_fb_resources));
	...
	shooteru_audio_init();
#endif
	shooteru_init_keypad();

	properties_kobj = kobject_create_and_add("board_properties", NULL);
	if (properties_kobj)
		rc = sysfs_create_group(properties_kobj,
				&shooteru_properties_attr_group);
...
static void __init shooteru_fixup(struct machine_desc *desc, struct tag *tags,
...
MACHINE_START(SHOOTERU, "shooteru")
#ifdef CONFIG_MSM_DEBUG_UART
	.phys_io = MSM_DEBUG_UART_PHYS,
	.io_pg_offst = ((MSM_DEBUG_UART_BASE) >> 18) & 0xfffc,
#endif
	.fixup = shooteru_fixup,
	.map_io = shooteru_map_io,
	.init_irq = msm8x60_init_irq,
	.init_machine = shooteru_init,
	.timer = &msm_timer,
MACHINE_END
